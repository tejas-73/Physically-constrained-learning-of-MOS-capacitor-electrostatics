...
parser.add_argument('t_ox', type=float, help='Enter the tox value in nm, mostly used while inference')
parser.add_argument('N_A', type=float, help='Enter the N_A value as a coefficient to 1e24, mostly used while inference')
parser.add_argument('train_samples', type=int, help='Number of Training Samples, used during inference. Therese are the number of equally spaced samples for y')
parser.add_argument('batch_size', type=int, help='Batch Size')
parser.add_argument('lr', type=float, help='initial learning rate of the model')
...
if parse.do_training and not parse.train_continue:
    ...
    code_file = open('./A_ODE_all_tox_NA.py') #This will copy the complete code in this file and save as a backup!
    code = ""
               ...
               +"7) tsi was taken as 50nm\n"
                +"8) N_A was taken as 1e24\n"
                 +"9) Model was trained with data dependent statistics\n"
                  +"10) Model doesnot have MSE loss only.\n"
                      +"11) Model optimizer has no weight decay\n"
                       +"12) Model with variable tox\n"
                      + "13) Model has variable N_A\n"
                      + "14) Model has no t_ox and N_A as graph nodes\n"
                        +"15) Model has N_A in 1e23 to 1e24\n"
                      +"16) Model has no variable exponential\n"
                       +"17) With NO univariate newton update\n")
...
if __name__ == '__main__':
        ...
        for epoch in range(n_epochs):
            ...
            if epoch == 0:
                ...
                for index, (y, vgs) in enumerate(train_loader):
                    ...
                    indices1 = np.round_(np.random.uniform(0, 13, size=(y.shape[0], 1)))
                    y = y.to(device)
                    vgs = vgs.to(device)
    ...
    np.random.seed(1000)
    try:
        model = torch.load(f'Poisson_model_model_final.pth').to(device)
    except:
    ...
    if parse.batch_size == -1:  # these are used when we need charge, $\eta_0$ and $V_T$ characteristics
        ...
        for vgs in tqdm(volt_array):
            ...
            predictions_model = model(y_prime, vgs * torch.ones_like(y_prime, dtype=torch.float64),
                                      ...
                                      parse.N_A * torch.ones_like(y_prime,
                                                                  dtype=torch.float64)).detach().cpu().numpy()  # model predictions
            int_val_list.append(
                integrate(model, y, vgs, parse.t_ox, parse.N_A.detach().cpu().numpy().astype(np.float64).reshape(-1, ),
                          discrete=predictions_model.reshape((-1,)).astype(
                              np.float64)))  # saving the integration values
            fem_val_list.append(
                integrate(model, y, vgs, parse.t_ox, parse.N_A.detach().cpu().numpy().astype(np.float64).reshape(-1, ),
    ...
    if parse.batch_size == 0:
        ...
        array = v_t.item() * np.array(
            [-3, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 3])  # taking values of those voltages that we are interested in
    ...
    for index, Vgs in enumerate(array):
        ...
        test_surface_vgs = torch.round(
            (torch.linspace(-4.5 * v_t, 4.5 * v_t, 300, dtype=torch.float64)).reshape((-1, 1)),
            decimals=5).to(device) #For checking
        ...
        if psi0_required:
            np.save(f'./{parse.save_model_tag}_vgs.npy', test_surface_vgs[
                -1] + vgs_width)  # saving the voltage for the case that it can be harnessed by other scripts
            ...
            surface_potential_derivative_true = list(
                np.diff(np.array(psi0_list, dtype=np.float64).reshape((-1, 1)), axis=0) / (
                    vgs_width))  # We get here numerical surface potential discrete derivative
            ...
            surface_potential_second_derivative_true = list(
                np.diff(np.diff(np.array(psi0_list, dtype=np.float64).reshape((-1, 1)), axis=0),
                        axis=0) / vgs_width ** 2)  # We get here numerical surface potetial discrete double derivative
            psi0_list.pop()
            psi0_list.pop()
        ...
        if parse.batch_size == 0:
            ...
            df.to_csv(
                f'vgs={parse.Vgs}_{parse.save_model_tag}.csv')  # saving the potential profile with absolute error in an excel file.
            df = pd.DataFrame(
                {'vgs': test_surface_vgs.reshape(-1, ), 'predicted surface potential': surface_potential.reshape(-1, )})
            df.to_csv(
                f'surface_potential_only_predictions_{parse.save_model_tag}.csv')  # saving the surface potential predictions in a excel file.
            plot_many_sp(model,
                         v_t)  # plotting various predictions for surface potential with different values of t_ox and N_A. see from where this function is called.
            print(
            ...
            print(
                f'Saved the surface potential predictions for vgs in -{parse.Vgs}vt to {parse.Vgs}vt, and saved as !!surface_potential_only_predictions_{parse.save_model_tag}.csv!!')
            ...
            predict(model, None, parse.Vgs * v_t, parse.t_ox, parse.N_A,
                    ...
                    if_use_y=torch.linspace(0, t_si, 3000, dtype=torch.float64).reshape(-1, ).to(
                        device), psi_b=psi_F.item())  # saving all the predictions in an excel file. This is saving again the profile.
        ...
        if parse.batch_size == 2 and parse.batch_size != 0:
            ...
            if index == len(array) - 1:
                ...
                plt.tight_layout()
                plt.xlim(-1, 60)
                plt.savefig('zoom_in_Poisson_Model_Prediction_vs_actual_all.png',
                            dpi=200)  # zoomed in version of the potential profile \Psi (y) vs numerical solution
                plt.xlim(-1, 205)
                plt.text(140, -0.3, f'Epoch: {check_epoch}', fontsize=13)
                plt.savefig('Poisson_Model_Prediction_vs_actual_all.png',
                            dpi=200)  # potential profile \Psi (y) vs numerical solution
                plt.close()
        ...
        plt.grid(True)
        plt.savefig(
            "Poisson_Model_Prediction_vs_actual.png")  # for plotting for a particular V_G, prediction vs numerical
        plt.close()
        ...
        plt.grid(True)
        plt.savefig(
            "Poisson_Model_Prediction_vs_actual_derivative.png")  # for plotting for a particular V_G first derivative, prediction vs numerical
        plt.close()
        ...
        plt.savefig(
            "Poisson_Model_Prediction_vs_actual_second_derivative.png")  # for plotting for a particular V_G second derivative, prediction vs numerical
        plt.close()
        ...
        create_document(parse.save_model_tag,
                        [f'./Poisson_Model_Prediction_vs_actual.png',
                         f'./Poisson_Model_Loss.png'])  # This adds all requested figure names to a document to scrutinizingly observe.
        ...
        plt.legend(['Loss Curve'])
        plt.savefig(
            "Poisson_Model_Loss_derivative.png")  # This is for the loss incurred in the first derivative as compared with the predicted solution
        plt.close()
        ...
        if index == len(array) - 1 and parse.batch_size != 0:
            ...
            plt.grid(True)
            plt.savefig(
                "Surface_Potential.png")  # Saving the normalised surface potential for prediction vs numerical solution
            plt.close()
            ...
            plt.grid(True)
            plt.savefig(
                "Surface_Potential_absolute_error.png")  # saving surface potential error observed in terms of log10(error)
            plt.close()
            ...
            plt.grid(True)
            plt.savefig(
                "Surface_Potential_derivative.png")  # saving the ML model prediction and numerical solution prediction for surface potential first derivative
            plt.close()
            ...
            pd.DataFrame({'Vgs': np.array(test_surface_vgs).reshape(-1, ),
                          ...
                          'Psi0_NN': np.array(surface_potential).reshape(-1, )}).to_csv(
                f'SP_derv.csv')  # saving the surface potential and its derivative values in a csv file.
            ...
            create_document(parse.save_model_tag,
                            [f'./Surface_Potential.png', f'./Surface_Potential_absolute_error.png',
                             f'Surface_Potential_derivative.png',
                             f'Surface_Potential_second_derivative.png'])  # putting the required stuffs in a document
