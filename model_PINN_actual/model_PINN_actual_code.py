                 ...
                 +"9) Model was trained with data dependent statistics\n"
                  +"10) Model doesnot have L1 loss with 3 x torch.sqrt before.\n"
                      ...
                      +"16) Model has no variable exponential\n"
                       +"17) PINN model, with $\lambda_1 = 10^{16}$ and $\lambda_2 = 10^{34}$")
    ...
    if do_training:
        ...
        for epoch in range(n_epochs):
            ...
            if epoch == 0:
                ...
                for index, (y, vgs) in enumerate(train_loader):
                    ...
                    with torch.no_grad():
                        ...
                        check_loss += (loss(-(torch.exp(-pred/psi_t) - 1 - torch.exp(-2*psi_F/psi_t) * (torch.exp(pred/psi_t) - 1)) * A, pred2).to(device) + (1e16)*loss(-a*(vgs - pred_zero), pred1_zero).to(device) + (1e34)*loss(pred_t_si, torch.zeros_like(pred_t_si).to(device))).item()
            ...
            for index, (y, vgs) in enumerate(train_loader):
                ...
                loss_ = (loss(-(torch.exp(-pred/psi_t) - 1 - torch.exp(-2*psi_F/psi_t) * (torch.exp(pred/psi_t) - 1)) * A, pred2).to(device) + (1e16)*loss(-a*(vgs - pred_zero), pred1_zero).to(device) + (1e34)*loss(pred_t_si, torch.zeros_like(pred_t_si).to(device))).to(device)
    ...
    for index, Vgs in enumerate(array):
        ...
        if parse.batch_size == 2 and parse.batch_size != 0:
            ...
            if index == len(array) - 1:
                ...
                plt.xlim(-1, 205)
                plt.text(140, -0.3, f'Epoch: 50789', fontsize=13)
                plt.text(25, 2.25, '$\lambda_1 = 10^{16}$, $\lambda_2 = 10^{34}$')
