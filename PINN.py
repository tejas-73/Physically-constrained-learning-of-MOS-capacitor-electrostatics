    ...
    if do_training:
        ...
        for epoch in range(n_epochs):
            ...
            if epoch == 0:
                ...
                for index, (y, vgs) in enumerate(train_loader):
                    ...
                    with torch.no_grad():
                        ...
                        loss_main += (loss(-(torch.exp(-pred/psi_t) - 1 - torch.exp(-2*psi_F/psi_t) * (torch.exp(pred/psi_t) - 1)) * A, pred2)).to(device).item()
                        loss_bc1 += loss(-a*(vgs - pred_zero), pred1_zero).to(device).item()
                        loss_bc2 += loss(pred_t_si, torch.zeros_like(pred_t_si).to(device)).item()
                ...
                loss_main = np.sum(np.array(loss_list_main))
                loss_bc1 = np.sum(np.array(loss_list_bc1))
                loss_bc2 = np.sum(np.array(loss_list_bc2))
                batch_loss_list.clear()
                loss_list_bc1.clear()
                loss_list_bc2.clear()
                loss_list_main.clear()
            loss_list.append(check_loss)
            total_loss_list_bc1.append(loss_bc1)
            total_loss_list_bc2.append(loss_bc2)
            total_loss_list_main.append(loss_main)
            np.save(f'model_{parse.save_model_tag}_total_loss.npy', np.array(loss_list))
            np.save(f'model_{parse.save_model_tag}_loss_bc1.npy', np.array(total_loss_list_bc1))
            np.save(f'model_{parse.save_model_tag}_loss_bc2.npy', np.array(total_loss_list_bc2))
            np.save(f'model_{parse.save_model_tag}_loss_main.npy', np.array(total_loss_list_main))
            ...
            for index, (y, vgs) in enumerate(train_loader):
                ...
                loss_main = (loss(-(torch.exp(-pred / psi_t) - 1 - torch.exp(-2 * psi_F / psi_t) * (torch.exp(pred / psi_t) - 1)) * A, pred2)).to(device)
                loss_bc1 = loss(-a * (vgs - pred_zero), pred1_zero).to(device)
                loss_bc2 = loss(pred_t_si, torch.zeros_like(pred_t_si).to(device))

                loss_ = loss_main + l_1 * loss_bc1 + l_2 * loss_bc2
                ...
                loss_list_bc1.append(loss_bc1.item())
                loss_list_bc2.append(loss_bc2.item())
